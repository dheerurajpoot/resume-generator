    ...
    >
      <div className="p-8 print:p-6">
        ...
        <div className="text-center mb-8">
          ...
          {data.personalInfo.linkedin && <div className="text-sm text-gray-600 mt-1">{data.personalInfo.linkedin}</div>}
        </div>

        {/* Summary */}
        {data.summary && (
          <section className="mb-8">
            <h2 className="text-xl font-bold text-gray-900 mb-3 border-b-2 border-gray-300 pb-1">SUMMARY</h2>
                  ...
                  {data.education.map((edu) => (
                    <div key={edu.id}>
                      <div className="flex justify-between items-start mb-1">
                        ...
                        <span className="text-sm text-gray-600 whitespace-nowrap ml-4">{edu.duration}</span>
                      </div>
                ...
                <div className="space-y-2 text-sm">
                  {data.skills.frontend.length > 0 && (
                    <div>
                      ...
                      <span className="text-gray-700">{data.skills.frontend.join(", ")}</span>
                    </div>
                  )}
                  {data.skills.backend.length > 0 && (
                    <div>
                      <span className="font-semibold text-gray-900">Backend: </span>
                      <span className="text-gray-700">{data.skills.backend.join(", ")}</span>
                    </div>
                  )}
                  {data.skills.database.length > 0 && (
                    <div>
                      <span className="font-semibold text-gray-900">Database: </span>
                      <span className="text-gray-700">{data.skills.database.join(", ")}</span>
                    </div>
                  )}
                  {data.skills.tools.length > 0 && (
                    <div>
                  ...
                  {data.skills.other.length > 0 && (
                    <div>
                      ...
                      <span className="text-gray-700">{data.skills.other.join(", ")}</span>
                    </div>
                  )}
                </div>
              </section>
                ...
                <div className="space-y-4">
                  {data.projects.map((project) => (
                    <div key={project.id}>
                      ...
                      <ul className="list-disc list-inside text-sm text-gray-700 space-y-1">
                        {project.description.map((desc, index) => desc.trim() && <li key={index}>{desc}</li>)}
                      </ul>
                    </div>
                  ))}
                </div>
    ...
    >
      <div className="p-8 print:p-6">
        ...
        <div className="text-center mb-6">
          ...
          <div className="text-sm text-black flex justify-center items-center gap-2 mb-4">
            {data.personalInfo.email && <span>{data.personalInfo.email}</span>}
            {data.personalInfo.email && data.personalInfo.linkedin && <span>|</span>}
            {data.personalInfo.linkedin && <span>{data.personalInfo.linkedin}</span>}
          </div>
          {data.personalInfo.website && <div className="text-sm text-black mb-4">{data.personalInfo.website}</div>}
        ...
        {data.summary && (
          <section className="mb-6">
            <h2 className="text-lg font-bold text-black mb-2 tracking-wide">SUMMARY</h2>
        ...
        {data.experience.length > 0 && (
          <section className="mb-6">
            <h2 className="text-lg font-bold text-black mb-2 tracking-wide">WORK EXPERIENCE</h2>
              ...
              {data.experience.map((exp) => (
                <div key={exp.id}>
                  <div className="flex justify-between items-baseline mb-2">
                    ...
                    <span className="text-black font-medium">{exp.duration}</span>
                  </div>
        ...
        {data.projects.length > 0 && (
          <section className="mb-6">
            <h2 className="text-lg font-bold text-black mb-2 tracking-wide">PROJECTS</h2>
            <div className="border-b-2 border-black mb-4"></div>
            <div className="space-y-4">
              {data.projects.map((project) => (
                <div key={project.id}>
        ...
        {data.education.length > 0 && (
          <section className="mb-6">
            <h2 className="text-lg font-bold text-black mb-2 tracking-wide">EDUCATION</h2>
            <div className="border-b-2 border-black mb-4"></div>
                ...
                <div key={edu.id}>
                  <div className="flex justify-between items-baseline">
                    <div>
                      ...
                      <p className="text-black italic">{edu.institution}</p>
                    </div>
                    <span className="text-black font-medium">{edu.duration}</span>
                  </div>
                  {edu.description && <p className="text-black mt-1">{edu.description}</p>}
                </div>
              ))}
            </div>
          </section>
        ...
        {(data.skills.frontend.length > 0 ||
          ...
          data.skills.other.length > 0) && (
          <section>
            <h2 className="text-lg font-bold text-black mb-2 tracking-wide">TECHNICAL SKILLS</h2>
            <div className="border-b-2 border-black mb-4"></div>
            <div className="space-y-2">
              {data.skills.frontend.length > 0 && (
                <div>
                  <span className="font-bold text-black">• Frontend: </span>
                  <span className="text-black">{data.skills.frontend.join(", ")}</span>
                </div>
              )}
              {data.skills.backend.length > 0 && (
                <div>
                  <span className="font-bold text-black">• Backend: </span>
                  <span className="text-black">{data.skills.backend.join(", ")}</span>
                </div>
              )}
              {data.skills.database.length > 0 && (
                <div>
                  <span className="font-bold text-black">• Database: </span>
                  <span className="text-black">{data.skills.database.join(", ")}</span>
                </div>
              )}
              {data.skills.tools.length > 0 && (
                <div>
                  <span className="font-bold text-black">• Tools & Platforms: </span>
                  <span className="text-black">{data.skills.tools.join(", ")}</span>
                </div>
              )}
              {data.skills.other.length > 0 && (
                <div>
                  <span className="font-bold text-black">• Other: </span>
                  <span className="text-black">{data.skills.other.join(", ")}</span>
                </div>
              )}
            </div>
          </section>
  ...
  const renderMinimalTemplate = () => (
    ...
    >
      <div className="p-8 print:p-6">
        ...
        {data.summary && (
          <section className="mb-6">
            <h2 className="text-lg font-bold text-blue-600 mb-2 border-b-2 border-blue-600 pb-1">CAREER SUMMARY:</h2>
            <p className="text-gray-700 leading-relaxed mt-3">{data.summary}</p>
          </section>
        )}
        ...
        {data.education.length > 0 && (
          <section className="mb-6">
            ...
            <div className="mt-3">
              <h3 className="font-semibold text-gray-800 mb-2">Academic Qualification</h3>
              <ul className="list-disc list-inside text-gray-700 space-y-1 ml-4">
                {data.education.map((edu) => (
                  <li key={edu.id}>
                    {edu.degree} from {edu.institution} in {edu.duration}
                  </li>
                ))}
              </ul>
            </div>
        ...
        {(data.skills.frontend.length > 0 ||
          ...
          data.skills.other.length > 0) && (
          <section className="mb-6">
            <h2 className="text-lg font-bold text-blue-600 mb-2 border-b-2 border-blue-600 pb-1">STRENGTHS:</h2>
            <ul className="list-disc list-inside text-gray-700 space-y-1 mt-3 ml-4">
              {data.skills.frontend.length > 0 && <li>Frontend Technologies: {data.skills.frontend.join(", ")}</li>}
              {data.skills.backend.length > 0 && <li>Backend Technologies: {data.skills.backend.join(", ")}</li>}
              {data.skills.database.length > 0 && <li>Database Management: {data.skills.database.join(", ")}</li>}
              {data.skills.tools.length > 0 && <li>Tools & Platforms: {data.skills.tools.join(", ")}</li>}
              {data.skills.other.length > 0 && <li>Other Skills: {data.skills.other.join(", ")}</li>}
            </ul>
          </section>
        )}
        ...
        {data.projects.length > 0 && (
          <section className="mb-6">
            <h2 className="text-lg font-bold text-blue-600 mb-2 border-b-2 border-blue-600 pb-1">KEY PROJECTS:</h2>
            <ul className="list-disc list-inside text-gray-700 space-y-2 mt-3 ml-4">
              {data.projects.map((project) => (
                <li key={project.id}>
                  <span className="font-semibold">{project.name}</span> - {project.technologies}
        ...
        {(data.personalInfo.fullName || data.personalInfo.email || data.personalInfo.phone) && (
          <section className="mb-6">
            <h2 className="text-lg font-bold text-blue-600 mb-2 border-b-2 border-blue-600 pb-1">PERSONAL DETAILS:</h2>
            <div className="mt-3 space-y-2 text-gray-700">
              {data.personalInfo.fullName && (
                <div className="flex">
                  <span className="w-32">Name:</span>
                  <span className="flex-1">: {data.personalInfo.fullName}</span>
                </div>
              )}
              {data.personalInfo.email && (
                <div className="flex">
                  <span className="w-32">Email:</span>
                  <span className="flex-1">: {data.personalInfo.email}</span>
                </div>
              )}
              {data.personalInfo.phone && (
                <div className="flex">
                  <span className="w-32">Phone:</span>
                  <span className="flex-1">: {data.personalInfo.phone}</span>
                </div>
              )}
              {data.personalInfo.website && (
                <div className="flex">
                  <span className="w-32">Website:</span>
                  <span className="flex-1">: {data.personalInfo.website}</span>
                </div>
              )}
              {data.personalInfo.linkedin && (
                <div className="flex">
                  <span className="w-32">LinkedIn:</span>
                  <span className="flex-1">: {data.personalInfo.linkedin}</span>
                </div>
              )}
            </div>
          </section>
        ...
        {/* Declaration */}
        <section>
          <h2 className="text-lg font-bold text-blue-600 mb-2 border-b-2 border-blue-600 pb-1">DECLARATION:</h2>
          <p className="text-gray-700 leading-relaxed mt-3">
            I {data.personalInfo.fullName || "[Your Name]"} declare that all the above statements are correct to the
              ...
              <div className="space-y-4 text-sm">
                {data.skills.frontend.length > 0 && (
                  <div>
                    ...
                    <div className="text-slate-300 leading-relaxed">{data.skills.frontend.join(" • ")}</div>
                  </div>
                )}
                {data.skills.backend.length > 0 && (
                  <div>
                    <h3 className="font-semibold text-slate-200 mb-2">Backend</h3>
                    <div className="text-slate-300 leading-relaxed">{data.skills.backend.join(" • ")}</div>
                  </div>
                )}
                {data.skills.database.length > 0 && (
                  <div>
                    <h3 className="font-semibold text-slate-200 mb-2">Database</h3>
                    <div className="text-slate-300 leading-relaxed">{data.skills.database.join(" • ")}</div>
                  </div>
                )}
                {data.skills.tools.length > 0 && (
                  <div>
                    <h3 className="font-semibold text-slate-200 mb-2">Tools & Platforms</h3>
                    <div className="text-slate-300 leading-relaxed">{data.skills.tools.join(" • ")}</div>
                  </div>
                )}
                {data.skills.other.length > 0 && (
                  <div>
                    <h3 className="font-semibold text-slate-200 mb-2">Other Skills</h3>
                    <div className="text-slate-300 leading-relaxed">{data.skills.other.join(" • ")}</div>
                  </div>
                )}
              </div>
            </section>
          ...
          {data.education.length > 0 && (
            <section>
              ...
              <div className="space-y-4 text-sm">
                {data.education.map((edu) => (
                  <div key={edu.id}>
                    <h3 className="font-semibold text-slate-200 leading-tight mb-1">{edu.degree}</h3>
                    <p className="text-slate-300 mb-1">{edu.institution}</p>
                    <p className="text-slate-400 text-xs mb-2">{edu.duration}</p>
                    {edu.description && <p className="text-slate-300 text-xs leading-relaxed">{edu.description}</p>}
                  </div>
                ))}
              </div>
            </section>
                  ...
                  <div key={exp.id}>
                    <div className="flex justify-between items-start mb-3">
                      <div className="flex-1">
                        <h3 className="font-bold text-slate-800 text-lg leading-tight mb-1">{exp.position}</h3>
                ...
                {data.projects.map((project) => (
                  <div key={project.id}>
                    <div className="mb-2">
                      ...
                      <p className="text-sm text-blue-600 font-medium italic">{project.technologies}</p>
                    </div>
                    <ul className="list-disc list-inside text-sm text-gray-700 space-y-1 ml-4 leading-relaxed">
                      {project.description.map((desc, index) => desc.trim() && <li key={index}>{desc}</li>)}
                    </ul>
